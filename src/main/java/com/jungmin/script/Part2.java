...
public class Part2 {
  ...
  */
  public static final String PART2_3 = "insert into users (username, email, password) value ('권택현', 'tjsk1999@gmail.com', '1q2w3e4r')";
  ...
  */
  public static final String PART2_10 = "insert into posts (title, content, user_id) value " +
          "('hoodcat', 'you nah mean', 1)";
  ...
  */
  public static final String PART2_12 = "select p.title from posts p join " +
          "users u on u.user_id = p.user_id where u.user_id = 1";

  /*
  ----------------------------------------------------------------------------------------------
      TODO: Q 2-13. users 테이블의 user_id가 1인 유저가 작성한 post의 board name을 찾기위한 SQL을 작성해주세요.
  */
  public static final String PART2_13 = "select b.name from boards b join " +
          "posts p on p.board_id = b.board_id " +
  ...
  */
  public static final String PART2_14 = "select p.title, p.content, p.created_at from posts p " +
          "join boards b on b.board_id = p.board_id where b.name = 'General Discussion'";

  /*
  ----------------------------------------------------------------------------------------------
      TODO: Q 2-15. luckykim이 작성한 comment의 개수 (컬럼명: CommentCount)를 출력하기 위한 SQL을 작성해주세요.
  */
  public static final String PART2_15 = "select count(c.comment_id) as CommentCount from comments c " +
          "join users u on u.user_id = c.user_id where u.username = 'luckykim'";
  ...
  */
  public static final String PART2_16 = "select u.username as username, count(c.comment_id) as CommentCount from comments c " +
          "right join users u on u.user_id = c.user_id group by u.username";

  /*
  ----------------------------------------------------------------------------------------------
      TODO: Q 2-17. 각 board의 name과 해당 board에 속한 post의 개수 (컬럼명: PostCount)를 출력하기 위한 SQL을 작성해주세요.
  */
  public static final String PART2_17 = "select b.name, count(post_id) as PostCount from posts p " +
          "join boards b on b.board_id = p.board_id group by b.name";

  /*
  ----------------------------------------------------------------------------------------------
      TODO: Q 2-18. 각 board의 name과 해당 board에 속한 posts 테이블의 content의 평균 길이 (컬럼명: AvgLength)를 출력하기 위한 SQL을 작성해주세요.")
  */
  public static final String PART2_18 = "select b.name, avg(length(p.content)) as AvgLength from posts p " +
          "right join boards b on b.board_id = p.board_id group by b.name";
  ...
  */
  public static final String PART2_19 = "select b.name, avg(length(p.content)) as AvgLength from posts p " +
          "join boards b on b.board_id = p.board_id where p.content is not null group by b.name";
